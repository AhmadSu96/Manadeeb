  ...
  Directions (from,to) {
    ...
    const url = `https://maps.googleapis.com/maps/api/directions/json?origin=${origin}&destination=${destination}&key=${APIKEY}&mode=${mode}`;
     ...
     fetch(url).then(response => response.json())
               .then(responseJson => {
                 console.log(responseJson.routes.legs.distance.text)
                 var text = responseJson.routes.legs.distance.text.match(/\d+/)[0]
                 this.setState({ distance: text });
                 switch (text) {
                  case (text > 10):
                    this.setState({ Cust: "30 QAR" });
                     break;
                  case (text > 11 && text < 25):
                    this.setState({ Cust: "45 QAR" });
                      break;
                  case (text > 24 && text < 56) :
                    this.setState({ Cust: "65 QAR" });
                     break;
                  case (text > 55 ) :
                    this.setState({ Cust: "100 QAR" });
                    break;
          ...
          <View style={styles.serchContinar}>
            ...
            {this.state.coords &&
              <View style={styles.SubmitContinar}>
                <Test>The distance: {} </Test>
                <Test>The Cust : {this.state.Cust} </Test>
              </View>
              } 
        </View>
      </Layout>
